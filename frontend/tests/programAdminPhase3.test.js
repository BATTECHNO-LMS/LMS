...
describe('program_admin Phase 3 frontend runtime removal', () => {
  it('active admin role sets exclude program_admin', () => {
    assert.equal(ADMIN_ROLE_SET.includes(PROGRAM_ADMIN), false);
    assert.equal(ADMIN_SHELL_ROLES.includes(PROGRAM_ADMIN), false);
    assert.equal(ASSIGNABLE_USER_ROLE_CODES.includes(PROGRAM_ADMIN), false);
    assert.equal(FIELD_TRAINING_ADMIN_ROLES.includes(PROGRAM_ADMIN), false);
    assert.ok(LEGACY_DEPRECATED_ROLE_CODES.includes(PROGRAM_ADMIN));
    assert.equal(isLegacyDeprecatedRole(PROGRAM_ADMIN), true);
  });

  it('admin navigation excludes program_admin', () => {
    assert.deepEqual(flattenAdminNavPaths(PROGRAM_ADMIN), []);
  });

  it('route helpers deny program_admin (fail closed, not student)', () => {
    assert.equal(isAdminRole(PROGRAM_ADMIN), false);
    assert.equal(getDashboardPathForRole(PROGRAM_ADMIN), '/login');
    assert.equal(getDashboardPathForRole(ROLES.STUDENT), '/student/dashboard');
    assert.equal(canAccessPathWithUiPermissions(PROGRAM_ADMIN, '/admin/dashboard'), false);
    assert.equal(canAccessPathWithUiPermissions(PROGRAM_ADMIN, '/student'), false);
    assert.equal(
      canAccessPathWithUiPermissionsForUser({ role: PROGRAM_ADMIN }, '/admin/dashboard'),
      false
    );
  });

  it('user-management permissions deny program_admin', () => {
    assert.equal(canManageUsers({ roles: [PROGRAM_ADMIN], isGlobal: false }), false);
    assert.equal(canManageUsers({ roles: [ROLES.SUPER_ADMIN], isGlobal: true }), true);
  });

  it('historical labels and deprecated codes remain available', () => {
