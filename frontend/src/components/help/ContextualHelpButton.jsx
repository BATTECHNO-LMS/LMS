...
export function ContextualHelpButton({ contextualKey, route, className = '' }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['contextual-help', contextualKey, route],
    queryFn: () => fetchContextualHelp({ key: contextualKey, route }),
    enabled: open,
    staleTime: 120_000,
  });
        ...
        <div className="ug-drawer-backdrop" onClick={() => setOpen(false)} role="presentation">
          ...
          >
              ...
              <ul className="ug-drawer__list">
                {articles.map((a) => (
                  <li key={a.id}>
                    ...
                    <Link to={`/student/user-guide/articles/${a.slug}`} onClick={() => setOpen(false)}>
            ...
            <footer className="ug-drawer__footer">
              <Link className="btn btn--outline btn--sm" to="/student/user-guide" onClick={() => setOpen(false)}>
