/**
 * Arabic-first HTML email templates for BATTECHNO LMS transactional email.
 */

function buildEmailVerificationOtpHtml({ userName, otp, expiryMinutes }) {
  const safeName = escapeHtml(userName || 'عزيزي/عزيزتي الطالب');
...
<head>
  ...
  <title>رمز توثيق البريد الإلكتروني - BATTECHNO LMS</title>
</head>
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e8dcc8;box-shadow:0 8px 24px rgba(15,35,65,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,#0f2341 0%,#1a3a5c 100%);padding:28px 32px;text-align:center;">
              ...
              <h1 style="margin:8px 0 0;color:#ffffff;font-size:22px;font-weight:700;">توثيق البريد الإلكتروني</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:32px;color:#1a2332;font-size:15px;line-height:1.7;">
              <p style="margin:0 0 16px;">مرحبًا ${safeName}،</p>
              <p style="margin:0 0 20px;">رمز توثيق بريدك الإلكتروني هو:</p>
              ...
              <p style="margin:0 0 12px;color:#4a5568;">لا تشارك هذا الرمز مع أي شخص. فريق BATTECHNO LMS لن يطلب منك الرمز عبر الهاتف أو الرسائل.</p>
              <p style="margin:0;color:#718096;font-size:14px;">إذا لم تقم بإنشاء حساب في BATTECHNO LMS، يمكنك تجاهل هذه الرسالة.</p>
            </td>
          </tr>
          <tr>
...
function buildEmailVerificationOtpText({ userName, otp, expiryMinutes }) {
  const name = userName || 'عزيزي/عزيزتي الطالب';
  return `مرحبًا ${name},

رمز توثيق بريدك الإلكتروني هو: ${otp}

هذا الرمز صالح لمدة ${expiryMinutes} دقائق فقط.
لا تشارك هذا الرمز مع أي شخص.

إذا لم تقم بإنشاء حساب في BATTECHNO LMS، يمكنك تجاهل هذه الرسالة.
