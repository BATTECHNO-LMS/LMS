...
test('HTML template embeds Sakkal Majalla, officer name, and isolated student fields', () => {
  ...
  assert.match(htmlA, /info-label completion-info-label">اسم الطالب\/ة:<\/span>/);
  assert.match(htmlA, /info-value--ltr completion-info-value--ltr">FT-TEST-A<\/span>/);
  assert.match(htmlA, /\.info[\s\S]*direction:\s*rtl/);
  assert.match(htmlA, /\.info-row[\s\S]*justify-content:\s*flex-start/);
  ...
  assert.doesNotMatch(htmlA, /Times New Roman|Tajawal/i);
